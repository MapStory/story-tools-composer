...
function composerController(
  ...
  $rootScope.$on("updateStorypins", (event, chapters) => {
    ...
    for (let f=0; f<chapters[0].storyframes.length; f++) {
      const coords = JSON.parse(chapters[0].storyframes[f].center);
      $scope.frameSettings.push({
          title: chapters[0].storyframes[f].title,
          startDate: moment.unix(chapters[0].storyframes[f].start_time).format('YYYY-MM-DD'),
          endDate: moment.unix(chapters[0].storyframes[f].end_time).format('YYYY-MM-DD'),
